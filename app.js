...
function examHome(){
  ...
  document.getElementById('app').innerHTML=`<div class="box"><div class="ttl">本番10問</div><div class="q">${volText()} からランダムで10問</div><div class="sub">穴埋め・概要記入・四字熟語を混ぜて出題します。答えを記入したあと正答例を確認し、自分で「正解 / 不正解」を判定します。</div><div class="acts"><button class="pri" id="startExam">10問テストを開始</button></div>${last?`<div class="card"><div class="meta">直近 ${fmt(last.date)}｜${volText(last.volumes)}</div><div class="score">${last.score}/10点 ${delta===null?'':`<span class="delta ${delta>0?'up':delta<0?'down':'flat'}">${delta>0?'↑ +':delta<0?'↓ ':'→ '}${delta}点</span>`}</div></div>`:''}</div>`;
...
function showExam(){
  ...
  document.getElementById('app').innerHTML=`<div class="card"><div class="meta">本番10問｜${volText(run.volumes)}｜原本メモ 第${x[0]}巻｜${x[1]}｜${i+1}/10</div><div class="prog"><i style="width:${i*10}%"></i></div><div class="q">${x[2]}</div>${x[1]==='穴埋め'?'<input id="examInput" placeholder="答えを入力">':'<textarea id="examInput" placeholder="概要・重要なことを自分の言葉で記入"></textarea>'}<div class="acts"><button class="pri" id="reveal">答え合わせ</button></div><div class="ans" id="an"><b>正答例：</b>${x[3]}<div class="exp">${x[4]}</div></div><div class="acts" id="judge" style="display:none"><button class="pri" data-j="ok">○ 正解</button><button class="b danger" data-j="ng">× 不正解</button></div></div>`;
  let inp=document.getElementById('examInput');inp.focus();
  document.getElementById('reveal').onclick=()=>{if(!inp.value.trim()&&!confirm('未記入のまま答えを見ますか？'))return;document.getElementById('an').classList.add('show');document.getElementById('judge').style.display='flex';document.getElementById('reveal').style.display='none';inp.disabled=true};
...
function showPractice(t){
  locked=false;let x=quiz[i];
  document.getElementById('app').innerHTML=`<div class="card"><div class="meta">${t}｜原本メモ 第${x[0]}巻｜${x[1]}｜${i+1}/${quiz.length}</div><div class="prog"><i style="width:${i/quiz.length*100}%"></i></div><div class="q">${x[2]}</div><div class="acts"><button class="pri" id="rev">答え・要点を見る</button></div><div class="ans" id="an"><b>答え：</b>${x[3]}<div class="exp">${x[4]}</div></div><div class="acts" id="grade" style="display:none"><button class="b" data-g="ok">できた</button><button class="b" data-g="mid">あやしい</button><button class="b" data-g="ng">できなかった</button></div></div>`;
  document.getElementById('rev').onclick=()=>{document.getElementById('an').classList.add('show');document.getElementById('grade').style.display='flex';document.getElementById('rev').style.display='none'};
