...
function stats(){
  st.textContent=F().length;
  sd.textContent=H.length;
  if(H.length){let avg=H.reduce((a,h)=>a+h.score,0)/H.length;sr.textContent=avg.toFixed(1)+'/10';sw.textContent=H[0].score+'/10'}else{sr.textContent='—';sw.textContent='—'}
}
function padHTML(){return `<div class="handbox"><div class="handlabel">手書き回答 <span>漢字も書いて覚える</span></div><canvas id="handpad" class="handpad"></canvas><div class="acts handacts"><button class="b" id="undoStroke">↶ 1画戻す</button><button class="b" id="eraser">消しゴム</button><button class="b danger" id="clearPad">全部消す</button></div></div>`}
function setupPad(){
  ...
  function redraw(){const r=c.getBoundingClientRect();ctx.clearRect(0,0,r.width,r.height);ctx.fillStyle='#fff';ctx.fillRect(0,0,r.width,r.height);ctx.strokeStyle='#eef1f6';ctx.lineWidth=1;ctx.setLineDash([4,5]);ctx.beginPath();ctx.moveTo(r.width/2,0);ctx.lineTo(r.width/2,r.height);ctx.moveTo(0,r.height/2);ctx.lineTo(r.width,r.height/2);ctx.stroke();ctx.setLineDash([]);strokes.forEach(drawStroke)}
  ...
  const undo=document.getElementById('undoStroke'),clear=document.getElementById('clearPad'),er=document.getElementById('eraser');undo.onclick=()=>{strokes.pop();redraw()};clear.onclick=()=>{strokes=[];redraw()};er.onclick=()=>{erase=!erase;er.classList.toggle('on',erase);er.textContent=erase?'✎ ペンに戻す':'消しゴム'};
  resize();setTimeout(resize,50);return{hasInk:()=>strokes.some(s=>!s.erase&&s.pts.length>1)}
}
...
function showExam(){
  ...
  reveal.onclick=()=>{
    if(hand){if(!pad.hasInk()&&!confirm('まだ何も書いていません。答えを見ますか？'))return;document.querySelectorAll('.handacts button').forEach(b=>b.disabled=true)}
    else{if(!inp.value.trim()&&!confirm('未記入のまま答えを見ますか？'))return;inp.disabled=true}
    an.classList.add('show');judge.style.display='flex';reveal.style.display='none'
  };
...
function practice(a,t){
  if(!a.length){app.innerHTML='<div class="box empty">対象問題がありません。</div>';return}
...
function showPractice(t){
  ...
  rev.onclick=()=>{
    ...
    an.classList.add('show');grade.style.display='flex';rev.style.display='none'
  };
...
function learn(){
  app.innerHTML='<div class="box"><div class="ttl">学習カード</div><input id="sea" placeholder="キーワード検索（例：5S、君子豹変、AIDEES）"><div id="cards"></div></div>';
  const sea=document.getElementById('sea'),cards=document.getElementById('cards');let p=()=>{let k=sea.value.toLowerCase(),a=F().filter(x=>!k||(x[2]+x[3]+x[4]).toLowerCase().includes(k));cards.innerHTML=a.length?a.map((x,n)=>`<div class="card"><div class="meta">原本メモ 第${x[0]}巻｜${x[1]}</div><div class="q">${x[2]}</div><div class="ans" id="a${n}"><b>答え：</b>${x[3]}<div class="exp">${x[4]}</div></div><div class="acts"><button class="b" onclick="document.getElementById('a${n}').classList.toggle('show')">答えを見る</button></div></div>`).join(''):'<div class="empty">該当なし</div>'};sea.oninput=p;p()
}
function aggregate(){
...
function historyView(){
  if(!H.length){app.innerHTML='<div class="box empty">本番10問の記録はまだありません。</div>';return}
  ...
  app.innerHTML=`<div class="box"><div class="ttl">点数の推移</div><div class="sub">最新${Math.min(H.length,20)}回を表示。↑↓は前回比です。</div><div class="history">${rows}</div></div>
